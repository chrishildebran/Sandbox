...
public class asdfasdf
{
	...
	{
		var propertyInfos = this.GetType().GetProperties().Where(t => t.PropertyType == typeof(ExternalParameterDefinitions));
	...
	public void PerformReflectionOnPOCO(object o)
	{
		...
		foreach (var propertyInfo in propertyInfos)
		{
			...
			if (propertyInfo.GetType().IsClass && !(propertyValue is IEnumerable) && !(propertyInfo.PropertyType == typeof(DateTime)) && !propertyInfo.PropertyType.IsEnum)
			{
			...
			if (propertyInfo.GetType().IsClass && propertyValue is IEnumerable && !(propertyValue is string))
			{
				...
				if (propertyValue.GetType().Name == "List`1")
				{
					...
					foreach (var listItem in propertyValue as IEnumerable)
					{
